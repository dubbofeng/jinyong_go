...
// include: /Users/zhiyufeng/dev/KataGo-browser/cpp/pre.js
...
class GraphModelWrapper {
  ...
  predict(batches, inputBuffer, boardWxH, inputBufferChannels, inputGlobalBuffer, inputGlobalBufferChannels, values, miscvalues, ownerships, policies) {
    return Asyncify.handleSleep(async wakeUp => {
      try {
        // 在 pthread Worker 中使用全局 HEAPF32，否则使用 Module.HEAPF32
        const heapF32 = (typeof HEAPF32 !== 'undefined') ? HEAPF32 : Module.HEAPF32;
        
        if (!heapF32 || !heapF32.buffer) {
          console.error("[predict] HEAPF32 not available:", {
            hasGlobalHEAPF32: typeof HEAPF32 !== 'undefined',
            hasModuleHEAPF32: typeof Module !== 'undefined' && typeof Module.HEAPF32 !== 'undefined',
            isPthread: typeof ENVIRONMENT_IS_PTHREAD !== 'undefined' ? ENVIRONMENT_IS_PTHREAD : 'unknown'
          });
          throw new Error("HEAPF32.buffer is not available");
        }
        
        let bin_inputs = new Float32Array(heapF32.buffer, inputBuffer, batches * boardWxH * inputBufferChannels);
        let global_inputs = new Float32Array(heapF32.buffer, inputGlobalBuffer, batches * inputGlobalBufferChannels);
        ...
        for (i = 0; i < results.length; i++) {
          ...
          switch (result.size) {
           case 3:
            //value
            heapF32.set(data, values / heapF32.BYTES_PER_ELEMENT);
            break;

           case miscvaluesSize:
            // miscvalues
            heapF32.set(data, miscvalues / heapF32.BYTES_PER_ELEMENT);
            break;

           case boardWxH:
            // ownership
            heapF32.set(data, ownerships / heapF32.BYTES_PER_ELEMENT);
            break;

           case (boardWxH + 1) * 2:
            // policy
            heapF32.set(data, policies / heapF32.BYTES_PER_ELEMENT);
            break;
          }
        }
        wakeUp(1);
...
if (ENVIRONMENT_IS_PTHREAD || Module["ENVIRONMENT_IS_PTHREAD"]) {
  // 只在第一个 Worker 中加载 TensorFlow.js，避免重复声明
  // 使用全局标记确保即使多个Worker也只加载一次
  if (typeof self.TFJS_LOADED === 'undefined' && typeof tf === 'undefined') {
    self.TFJS_LOADED = true;
    if (isLocalTest()) {
      console.log("[Worker] Loading TensorFlow.js from local files...");
      try {
        importScripts("/katago/tfjs/tf.min.js", "/katago/tfjs/tf-backend-webgpu.min.js", "/katago/tfjs/tf-backend-wasm.min.js");
        console.log("[Worker] TensorFlow.js loaded, tf =", typeof tf);
        tf.wasm.setWasmPaths("/katago/tfjs/");
      } catch (e) {
        console.error("[Worker] Failed to load TensorFlow.js:", e);
        self.TFJS_LOADED = false;
      }
    } else {
      const version = "4.16.0";
      importScripts(`//cdn.jsdelivr.net/npm/@tensorflow/tfjs@${version}/dist/tf.min.js`, `//cdn.jsdelivr.net/npm/@tensorflow/tfjs-backend-wasm@${version}/dist/tf-backend-wasm.min.js`, `//cdn.jsdelivr.net/npm/@tensorflow/tfjs-backend-webgpu@${version}/dist/tf-backend-webgpu.min.js`);
      tf.wasm.setWasmPaths(`//cdn.jsdelivr.net/npm/@tensorflow/tfjs-backend-wasm@${version}/dist/`);
    }
  } else {
    console.log("[Worker] TensorFlow.js already loaded, skipping...");
  }
...
// end include: /Users/zhiyufeng/dev/KataGo-browser/cpp/pre.js
var arguments_ = [];
...
function ___syscall_readlinkat(dirfd, path, buf, bufsize) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 0, 1, dirfd, path, buf, bufsize);
  try {
    path = SYSCALLS.getStr(path);
    path = SYSCALLS.calculateAt(dirfd, path);
...
function ___syscall_rmdir(path) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 0, 1, path);
  try {
    path = SYSCALLS.getStr(path);
    FS.rmdir(path);
    return 0;
...
function ___syscall_unlinkat(dirfd, path, flags) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 0, 1, dirfd, path, flags);
  try {
    path = SYSCALLS.getStr(path);
    path = SYSCALLS.calculateAt(dirfd, path);
    if (!flags) {
...
function _fd_seek(fd, offset, whence, newOffset) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(19, 0, 1, fd, offset, whence, newOffset);
...
function _setBackend() {
  console.log("setBackend. should not reach");
}

function stdio_support() {
...
var Asyncify = {
  instrumentWasmImports(imports) {
    var importPattern = /^(emscripten_sleep|setBackend|downloadMetadata|downloadModel|waitForStdin|predict|invoke_.*|__asyncjs__.*)$/;
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ downloadMetadata: _downloadMetadata,
    /** @export */ downloadModel: _downloadModel,
    ...
    /** @export */ getBackend: _getBackend,
    /** @export */ jsGetModelVersion: _jsGetModelVersion,
    /** @export */ memory: wasmMemory,
    /** @export */ notifyStatus: _notifyStatus,
    /** @export */ predict: _predict,
    /** @export */ removeModel: _removeModel,
    /** @export */ setBackend: _setBackend,
    /** @export */ waitForStdin: _waitForStdin
  };
}
